...
void CSerialPort::ReceiveStr(CSerialPort* port)
{
	...
	for (;;)
	{
		...
		memcpy(port->read_data, RXBuff, BytesRead);
		port->IsReadSuccess = true;
...
unsigned int itas109::CSerialPort::Receive(CString & strRev, int timeout)
{
	int ret = WaitForSingleObject(m_hRecvEvent, timeout);
	if (WAIT_OBJECT_0 == ret)
	{
		ResetEvent(m_hRecvEvent);
		IsReadSuccess = false;
		CString str(read_data);
		strRev = str;
	}
	return ret;
}

void itas109::CSerialPort::Send(char * str)
{
	if (!IsOpened())
	{
		return;
	...
	ResumeMonitoring();
	WriteToPort(str, strlen(str));//发送
}

void itas109::CSerialPort::Send(char * strSend, int len)
{
	if (!IsOpened())
	{
		return;
