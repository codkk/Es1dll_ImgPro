...
**
************************************************************************************
**  author: wuhq date:2017-12-23
**
**  改进
**	1) 增加 Send 与 Receive 方法，阻塞式接收
**************************************************************************************
************************************************************************************
...
namespace itas109{
	...
	public:
		...
		unsigned int Receive(CString &strRev, int timeout = 1000); //接收数据； strRev:接收的内容；timeout：等待超时时间(ms)； 返回值：
		void		 Send(char* strSend);						   //发送数据：strSend:发送的内容 
		void		 Send(char* strSend,int len);						   //发送数据：strSend:发送的内容 
	...
	private:
		...
		char				read_data[1024]; //收到数据
		bool				IsReadSuccess;	 //是否读取成功
		HANDLE				m_hRecvEvent;	 //接收完毕事件
		// Event array. 
